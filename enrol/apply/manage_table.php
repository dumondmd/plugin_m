...
class enrol_apply_manage_table extends table_sql {
    ...
    public function __construct($enrolid = null) {
        ...
        $this->set_sql(
           "ue.id AS userenrolmentid, ue.userid, ue.status AS enrolstatus,
            ue.timecreated AS applydate,
            ai.comment AS applycomment, u.*, c.fullname as course, 'link'",
    ...
    public function col_applydate($row) {
        return date("d-m-Y", $row->applydate);
    }

    public function col_link($row) {
        //return html_writer::link(new moodle_url('/local/costcenter/courses.php', 'teste', array('id' => 'a', 'target'=>'_blank')));
        return html_writer::link(new moodle_url('/enrol/apply/buscausuario.php', array('id' => $row->userid)), 'Analisar', array('target' => '_blank'));
    }
}
