...
$url_l = new moodle_url('/blocks/acompanhamento/recursos/');
$PAGE->set_url($url);
...
if(empty($idUser) || empty($idCurso)){
  redirect($CFG->wwwroot . '/blocks/inscricao/view.php?idcurso='.$idCurso);
}
...
$usr = null;
$usr = getUser($idUser, $idCurso);

function getUser($id, $idcurso)
{
  global $DB;
  try {
...
<div class="form-row">
   <div class="form-group col-md-12">
      <a href="'.$url_upload.''.$usr->link_upload_comprovante_endereco.'" target="_blank"><i class="fa fa-file-pdf-o fa-2x"></i> <strong>Downlod comprovante de endereço</strong></a>
   </div>
</div>
...
<div class="form-row" id="descPis">
  ...
  if(empty(!$usr->link_upload_pis)){
    echo '
      <div class="form-group col-md-6 mt-4" id="numPISDesc">
...
<div class="form-row">
   <div class="form-group col-md-12">
      <a href="'.$url_l.''.$usr->link_upload_recurso.'" target="_blank"><i class="fa fa-file-pdf-o fa-2x"></i> <strong>Downlod recurso</strong></a>
   </div>
</div>
