...
foreach ($inscricaodb as $val) {
  ...
  gerarPDF($html, $val->firstname, $val->lastname);
...
function gerarPDF($html, $firstname, $lastname)
{
  $dompdf = new Dompdf();
  $dompdf->loadHtml($html);
  $dompdf->setPaper('A4', 'portrait');
  $dompdf->render();
  $nameFile = formatarNomeL($firstname, $lastname) . '_Caderno_de_prova';
  $output = $dompdf->output();
  file_put_contents('public/temporarios/' . $nameFile . '.pdf', $output);
}

function formatarNomeL($firstname, $lastname){
...
function zipArquivo($nomeCurso)
{

  $localNomeZip = 'public/compactados/' . $nomeCurso . '.zip';
  $localPdfs = "public/temporarios/";
  ...
  $zip->open($localNomeZip);

  //Listar arquivos
  $arquivos = scandir($localPdfs);
  unset($arquivos[0], $arquivos[1]);

  foreach ($arquivos as $arquivo) {
    $zip->addFile("{$localPdfs}{$arquivo}", $arquivo);
  }
...
function removerArquivosTemporarios($idcurso)
{
  ...
  //Deletar
  foreach ($arquivos as $arquivo) {
    unlink("{$localPdfs}{$arquivo}");
  }

  //Voltar quando concluir tudo
  $linkVoltar = new moodle_url('/blocks/inscricao/inscricoes/deferidas/view.php?idcurso=' . $idcurso);
...
//Criacao de arquivo zip
zipArquivo($curso_db->fullname);


//Remoção arquivos temporarios
removerArquivosTemporarios($idcurso);
